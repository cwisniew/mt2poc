...
class MapViewPortImplTest {
  ...
  void panViewLeft() {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      Rectangle2D expected =
          new Rectangle2D(current.getMinX() - panStep, current.getMinY(), width, height);
  ...
  void panViewUp() {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      Rectangle2D current = viewPort.getViewBounds();
      Rectangle2D expected =
          new Rectangle2D(current.getMinX(), current.getMinY() + panStep, width, height);
      viewPort.panViewUp();
  ...
  void getCorner() {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      assertEquals(
          ...
          viewPort.getCorner(MapViewCorner.TOP_LEFT),
          "Top Left");
      assertEquals(
          new Point2D(bounds.getMaxX(), bounds.getMaxY()),
          viewPort.getCorner(MapViewCorner.TOP_RIGHT),
          "Top Right");
      assertEquals(
          new Point2D(bounds.getMaxX(), bounds.getMinY()),
          viewPort.getCorner(MapViewCorner.BOTTOM_RIGHT),
          "Bottom Right");
      assertEquals(
          new Point2D(bounds.getMinX(), bounds.getMinY()),
          viewPort.getCorner(MapViewCorner.BOTTOM_LEFT),
          "Bottom Left");
    }
  }
